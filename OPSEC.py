...
def checkVersion():
	...
	if int(version) == 3:
		...
		sys.exit("\033[91m[!] use python version 3 ...\033[92m")
...
def loadlib():
	...
	import webbrowser
	import json
	import colorama # colorama for colorized texts
...
def emailSpam():
	try:
		email = input("[#][P9][SETool][Your email:~$ ")
		password = input("[#][P9][SETool][Your password:~$ ")
		print(wait+" Sign in ...")
		...
		try:
			...
			print("\033[92m[+] Connection completed!\033[92m\n")
		except:
			print("\033[91m[!] WRONG password \033[92m")
			sys.exit()
		spoof_mail = input("[#][P9][SETool][From:~$ ")
		target_email = input("[#][P9][SETool][To:~$ ")
		nb = input("[#][P9][SETool][NB Email to send:~$ ")
		nb = int(nb)

		objet = input("[#][P9][SETool][Objet:~$ ")
...
def facebookStalk():
	...
	helpMsgFbStalk = """\033[92m
		back : Return to the main menu.
		exit / quit  : To exit the software.
		clear : Clear screen."""

	resultProfile = """\033[92m
    [Name] %s
    [work] %s
    [Loc] %s
    [ID] %s"""
...
def bssidFinder():
	bssid = input("[#][P9][Lookup][MAC/BSSID:~$ ")
	# clear()
	...
	if code == "404":
		print("\n[%s]\n" % (bssid))
		print("\033[91m[!] Localization Not Found\033[92m")
	else:
		pass
...
def ipFinder():
	...
	if status != "success":
		print("\033[91m[!] IP not valid o.o'\033[92m")
...
def SearchEmail4():
	email = input("\n[#][P9][Lookup][Email:~$ ")
	print(wait+" Information search on '%s'" % (email))
	lkd = leaked()
	leak = lkd.email(email)

	if leak:
		print(found+" Site leak : "+leak)
		print(wait+" Password search ...")
...
def check_email_exist():
	...
	if code == 250:
		print(found+" %s exist." % (email_address))
	else:
		print(warning+" %s does not exist.." % (email_address))
	...
	for url in urls:
		...
		if not "googleusercontent" in url:
			if not "/settings/ads" in url:
				if not "/policies/faq" in url:
				# if "insta" in url or "twitter" in url or "facebook" in url:
					print("\033[92m[+] Possible connection: "+url)
...
def google():
	nom = input("\n[#][P9][Lookup][name, first name, city (...):~$ ")
...
def doxMaker():
	...
	nom = input("Last name : ")
	naissance = input("Date of Birth : ")
	telPortable = input("Phone number : ")
	email = input("Email : ")
	fax = input("Fax : ")
	telFixe = input("Home phone number :  ")
	ville = input("City : ")
	adresse = input("Address : ")
	cp = input("Postal Code : ")
	ip = input("IP Address : ")
	...
	f.write("\nFax: "+fax)
	f.write("\nCity: "+ville.title())
	f.write("\nAdrdess: "+adresse.title())
	f.write("\nPostal Code: "+cp)
	f.write("\nIP Address: "+ip)
	s = input("add social networks to the list? [Y/N]: ").upper()
	if s == 'Y':
		while True:
			reseaux = input(" Social media (ex: Facebook ...): ")
			name = input("Name, nickname, link : ")
			f.write("\n"+reseaux.capitalize()+': '+name)
			c = input(" Add an account? : [Y/N]: ").upper()
			if c == 'Y':
				pass
			else:
				break
		...
		pass
	s = input("Add other information? [Y/N]: ").upper()
	if s == 'Y':
...
def showDataBase():
	...
	def searchProfiles():
		...
		if nameType == 'ID' :
			...
			if nameFile is None:
				print(warning+"\033[91m No ID found.\033[92m")
			else:
			...
			if not find:
				...
				if not find:
					print("\033[91m[!] No profiles for\033[92m] '%s'." % (name))
...
"""
	...
	menuLogo = """\033[92m
This is YOUR data base not P9's, so you have to save files in here to have a data base.

"""
		
	menu = "\n\033[92m[s] Search Profiles     [a] Show all Profiles    [e] Exit Database    [c] Clear   [h] Help"
	...
	for data in datas:
		...
		if str(x) == '0':
			print("\033[91m[!] No profiles.\033[92m")
		...
		while True:
			...
			if choix.lower() == 'h':
				...
				print("\033[91mWRONG COMMAND\033[92m")
...
helpLookup = """\033[92m
 Commands Name                            Commands Action
 ---------------                  ---------------------------------------------------------
 Name Lookup                      Search by name, first name and (city).
 UserName Lookup                  Do some research with a pseudonym.
 Address Lookup                   Do research with an address.
 Phone Number Lookup              Do research with a phone number.
 IP Lookup                        Do research with IP Address.
 ...
 Exif data                        Do research using the MetaData of a photo.
 ...
 instagram info                   Retrieve information from a Instagram account.
 Hash decrypter                   Try to decrypt a hash via an online database.
 Back to main menu                Return to main menu.
 Exit                             Exit the software.
 Clear                            Clears Screen. Clears everything from Screen."""

helpSEtool = """\033[92m
 Commands Name                            Commands Action
 ---------------                  ---------------------------------------------------------
 SMS                              Receive SMS on free numbers.
 Spam email                       Spam an email address.
 Back to main menu                Return to main menu.
 Exit                             Exit the software.
 Clear                            Clears Screen. Clears everything from Screen."""
...
mainOption = """\033[92m
 ...
 [3] Make file for DataBase
 [4] Show DataBase
 [e] Exit    [h] Help    [c] Clear"""


lookupOption = """\033[92m
 [1] Name Lookup              [8] Mail tracer
 [2] UserName Lookup          [9] Exif data
 [3] Address Lookup           [10] Google search
 [4] Phone Number Lookup      [11] Facebook GraphSearch
 [5] IP Lookup                [12] twitter info
 [6] SSID Lookup              [13] instagram info
...
setoolOption = """\033[92m
 [1] SMS
 [2] Spam email
 [b] Back to main menu    [e] Exit    [c] Clear 
"""
...
def menu():
 ...
 _________        .-------            --------.                              | 
:______.-':      :  .-----------------------.  :                             | 
| ______  |      | :Date: %s \033[92m        | |                             | 
|:______B:|      | |Time: %s \033[92m          | |                             | 
|:______B:|      | |Performance: [%s\033[92m]       | |                             | 
|:______B:|      | |Loading OPSEC...         | |                             | 
|         |      | |STATUS: [\033[96mStable\033[92m]         | |                             | 
|:_____:  |      | |Connection: [%s\033[92m]          | |                             | 
|    ==   |      | :Loading successful...    : |                             | 
|       O |      :  '-----------------------'  :                             | 
|       o |      :'------..._________...------'                              | 
|       o |-._.-i______/'             \._                                    | 
...
:_________:      `.____________________   `-.___.-.                          | 
              ...
              :____________________________:                                 | 
                Made By AT ( SuitingBark92 )                                 |
      Some Stuff May Be Bugged, I'll Fix It When I Want                      |  
      I Mean If You Pay Me I'll Fix It ASAP, https://cash.app/$unknowns68    | 
-----------------------------------------------------------------------------I
                Welcome to P9'S MultiTool...
...
try:
	while True:
		...
		if choix.lower() == 'h':
			...
			sys.exit(information+" \033[92mJoin Discord:\033[96m https://discord.gg/9mN4QJqdGd\033[0m")
		elif choix == '1':
			clear()
			menu()
			print(lookupOption)
			while True:
				...
				#  ...
					...
					print("\033[91mWRONG COMMAND\033[0m")
		elif choix == '2':
			clear()
			menu()
			print(setoolOption)
